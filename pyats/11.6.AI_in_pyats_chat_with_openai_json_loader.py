...
class common_setup(aetest.CommonSetup):
    ...
    def loop_mark(self, testbed):
        aetest.loop.mark(Show_Route_Langchain, device_name=testbed.devices)

# Test Case
class Show_Route_Langchain(aetest.Testcase):
    """pyATS and AI"""
...
#        with open(f'{self.device.alias}_Show_IP_Route.json', 'w') as f:
#            f.write(json.dumps(self.parsed_json, indent=4, sort_keys=True))
#        log.info(f"Configuration saved to file: {self.device.alias}_Show_IP_Route.json")

#    @aetest.test
#    def load_text(self):
#        self.loader = JSONLoader( 
#            file_path=f'{self.device.alias}_Show_IP_Route.json', 
#            jq_schema=".info[]", 
#            text_content=False 
#        )
#        log.info(f"Loaded text from file: {self.device.alias}_Show_IP_Route.json")
...
#        log.info("Setup conversation memory.")
    ...
    @aetest.test
    def chat_with_show_route(self):
        try:
            # Load questions from the YAML file
            with open("show_route_questions.yaml", 'r') as file:
                data = yaml.safe_load(file)
                questions = data['questions']
            log.info(f"Loaded questions from show_route_questions.yaml.")

            # Open a file in write mode
            with open("Show_Route_Q_A.txt", 'w') as file:
                ...
                log.info("All questions and answers written to Show_Route_Q_A.txt.")
        except Exception as e:
            log.error(f"Error in chatting with show route: {str(e)}")
