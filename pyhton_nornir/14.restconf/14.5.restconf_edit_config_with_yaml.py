...
def restconf_edit_config_with_yaml(task):
    ...
    module2 = "Cisco-IOS-XE-native:native/router/bgp"
    url2 = f"https://{task.host.hostname}:443/restconf/data/{module2}"
    response = requests.put(url=url2, headers=headers, auth=(USERNAME, PASSWORD), verify=False, json=task.host["hdata"]["bgp_config"])
    print(response)

    module3 = "Cisco-IOS-XE-native:native/ntp"
    url3 = f"https://{task.host.hostname}:443/restconf/data/{module3}"
    response = requests.put(url=url3, headers=headers, auth=(USERNAME, PASSWORD), verify=False, json=task.host["hdata"]["ntp_config"])
    print(response)

    module4 = "openconfig-acl:acl"
    url4 = f"https://{task.host.hostname}:443/restconf/data/{module4}"
    response = requests.put(url=url4, headers=headers, auth=(USERNAME, PASSWORD), verify=False, json=task.host["hdata"]["acl_config"])
    print(response)
